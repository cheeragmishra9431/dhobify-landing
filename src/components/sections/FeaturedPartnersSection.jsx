    ...
    <Section id="featured-partners" background="white" className="scroll-mt-20 border-t border-gray-100">
      <Container>
        ...
        <ul className="mt-10 grid list-none gap-5 p-0 sm:grid-cols-2 lg:gap-6">
          {PARTNERS.map((p) => (
            <li key={p.name} className="rounded-card border border-gray-200 bg-brand-light-grey/25 p-5 md:p-6">
              ...
              <Link
                to={p.href}
                className="mt-4 inline-block text-sm font-semibold text-brand-black underline decoration-brand-yellow decoration-2 underline-offset-4"
              >
                View {p.name}
              </Link>
            </li>
